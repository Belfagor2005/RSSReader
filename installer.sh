...
exec > >(tee -a /tmp/RSSReader_debug.txt) 2>&1
set -x
...
wget --no-check-certificate 'https://github.com/Belfagor2005/RSSReader/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
...
IMAGE VERSION: $distro_version" >> /tmp/RSSReader_debug.txt
sleep 5
killall -9 enigma2
exit 0